...
import { CardFooter, Button } from '../../ui';  // Assurez-vous d'importer Button si vous l'utilisez
...
const CreateRoot: React.FC = () => {
    ...
    return (
        ...
        </div><div className="w-full space-y-3 md:flex md:space-x-3">
            ...
            </div><CardFooter className="flex justify-center border-t-2 border-indigo-100 shadow-inner bg-light">
                {showCreateForm ? (
                    <CreateForm onClose={handleCloseForm} title={"Create Dicom"} />
                ) : (
                    <Button color={Colors.success} onClick={handleCreateDicomClick}>
                        Create Dicom
                    </Button>
                )}
            </CardFooter></>
