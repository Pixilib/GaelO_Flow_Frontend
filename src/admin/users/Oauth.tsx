...
import { Oauth2Config, useCustomToast,useCustomQuery, useCustomMutation, Colors } from "../../utils";
...
const Oauth = () => {
    const [showOauthForm, setshowOauthForm] = useState<'create'| null>(null);
    const { toastSuccess, toastError } = useCustomToast();


    const { data: oauth2Config, isPending: isLoadingOauthConfig } = useCustomQuery<Oauth2Config[]>(
        ["oauth2Config"],
        () => getOauth2Config(),
        {
            enabled: true,
            ...
            <div className="flex justify-center mx-10">
                {
                  showOauthForm === null &&
                    <Button
                        ...
                        onClick={() => setshowOauthForm('create')}
                ...
                <div className="mt-4">
                    {showOauthForm === 'create' &&
                    <CreateOauth
                        title="Create Oauth Provider"
                        onClose={() => setshowOauthForm(null)}
                    />
                    }
                </div>
        </div>
