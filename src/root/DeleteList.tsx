...
const DeleteList = () => {
    ...
    return (
        ...
        <div className="flex items-center w-full gap-6 p-1 -mx-4 bg-primary rounded-2xl">
            ...
            {/* Export Button */}
            <span className="flex items-center px-4 py-0.5 bg-white rounded-md shadow-md hover:bg-orange-500 transition-colors group">
                <ExportIcon
                    className="mr-2 text-xl cursor-pointer text-secondary group-hover:text-white"
                    onClick={() => alert('Export action triggered!')}
                />
                <span className="px-2 py-0.5 rounded-full border border-gray-300 shadow-[0_4px_6px rgba(255,165,0,0.5)]">
                    {Object.keys(exportList).length}
                </span>
            </span>

            {/* Delete Button */}
            <span className="flex items-center px-4 py-0.5 bg-white rounded-md shadow-md hover:bg-red-500 transition-colors group">
                <DeleteIcon
                    className="mr-2 text-xl text-red-500 cursor-pointer group-hover:text-white"
                    onClick={() => alert('Delete action triggered!')}
                />
                <span className="px-2 py-0.5 rounded-full border border-gray-300 shadow-[0_4px_6px rgba(255,0,0,0.3)]">
