...
const EditStudy: React.FC<EditStudyProps> = ({
  studyId,
  onStudyUpdated,
  onClose,
  show,
  defaultKeepLabels = false,
}) => {
  const { toastSuccess, toastError } = useCustomToast();
  const [jobId, setJobId] = useState<string | null>(null);

  const { mutateAsync: mutateStudy } = useCustomMutation<
    any,
    { id: string; payload: StudyModifyPayload }
  >(
      ...
      <Modal.Body>
        {editingStudyDetails && (
          <StudyEditForm
            ...
            defaultKeepLabels={defaultKeepLabels}
