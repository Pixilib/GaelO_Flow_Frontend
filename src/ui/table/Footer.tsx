...
const Footer = ({ table }: FooterProps) => {
  ...
  return (
    <div className="flex justify-end w-full bg-white text-sm/3 rounded-b-xl">
      ...
      <div className="flex">
        <button className={`${btnClassFooter}`} color={Colors.orange} onClick={() => setPageIndex(0)} disabled={!getCanPreviousPage} aria-label="Go to the first page!">
          <MdSkipPrevious size="1.6em" />
        </button>
        
        <button className={`${btnClassFooter}`} color={Colors.dark} onClick={() => setPageIndex(Math.max(pageIndex - 1, 0))} disabled={!getCanPreviousPage} aria-label="Go to the previous page!">
        ...
        <button className={`${btnClassFooter}`} color={Colors.dark} onClick={() => setPageIndex(Math.min(pageIndex + 1, getPageCount() - 1))} disabled={!getCanNextPage} aria-label="Go to the next page!">
          <GrNext size="1.3em" />
        </button>
