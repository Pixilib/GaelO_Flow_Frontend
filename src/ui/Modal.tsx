...
const Title = ({ children, className= "", ...props }: TitleProps) => {
  return (
    <div data-gaelo-flow='modal-title' className={"relative text-3xl font-semibold text-primary dark:text-white" + className} {...props}>
      {children}
    </div>
  );
};
...
const Body = ({ className = "", children, ...props }: BodyProps) => {
  return <div data-gaelo-flow='modal-body' className={"relative w-full gap-x-2 py-3 px-4 overflow-auto " + className} {...props} >{children}</div>;
};
